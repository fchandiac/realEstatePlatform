...
const AdminCard: React.FC<AdminCardProps> = ({ admin, onEdit, onDelete }) => {
  ...
  <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-6 md:gap-4 items-stretch">
          ...
          <div className="flex flex-col gap-4 sm:gap-2 w-full overflow-hidden">
            ...
            {/* Nombre de usuario */}
            <p className="text-sm text-neutral-600 truncate break-all">@{admin.username}</p>

            {/* Correo con icono */}
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-neutral-500" style={{ fontSize: '1rem' }}>email</span>
              <p className="text-sm text-neutral-500 truncate break-all">{admin.email}</p>
            </div>
            ...
            {admin.personalInfo?.phone && (
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm text-neutral-500 truncate break-all">{admin.personalInfo.phone}</p>
              </div>
            )}
          </div>
        </div>
