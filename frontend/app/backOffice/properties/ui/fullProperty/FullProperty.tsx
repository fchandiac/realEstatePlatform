...
import { findUserById } from '@/app/actions/users';
...
const FullProperty: React.FC<FullPropertyDialogProps> = ({ propertyId, onSave }) => {
  ...
  // Estado para nombres de usuarios en el historial - YA NO ES NECESARIO, viene del servidor
  ...
  // ELIMINADO: useEffect que cargaba nombres de usuarios desde el cliente
  // Ahora los nombres vienen pre-cargados desde el servidor en changeHistory[].changedByName
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm text-muted-foreground">
                  {currentProperty.changeHistory?.length || 0} cambios registrados
                </div>
              </div>

              {!currentProperty.changeHistory?.length ? (
                <p className="text-muted-foreground">No hay cambios registrados.</p>
              ) : (
                <div className="space-y-4">
                  {currentProperty.changeHistory
...
// Funciones auxiliares para formatear el historial de cambios
...
async function formatValue(value: any, field: string): Promise<string> {
  if (value === null || value === undefined) return 'N/A';

  // Para campos que son IDs, intentar resolver nombres
  if (field === 'propertyTypeId' && typeof value === 'string') {
    // Aquí podrías hacer un fetch para obtener el nombre del tipo, pero por simplicidad:
    return `Tipo ID: ${value}`;
  }
  if (field === 'assignedAgentId' && typeof value === 'string') {
    const user = await findUserById(value);
    return user?.name || `Usuario ID: ${value}`;
  }

  // Formateo básico
  if (typeof value === 'boolean') return value ? 'Sí' : 'No';
  if (typeof value === 'number') return value.toLocaleString('es-ES');
  if (field === 'price' && typeof value === 'number') return `$ ${value.toLocaleString('es-ES')}`;
  return String(value);
}
